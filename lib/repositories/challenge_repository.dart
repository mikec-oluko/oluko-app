...
class ChallengeRepository {
  ...
  static Future<List<Challenge>> getUserChallengesBySegmentId(String segmentId, String userId) async {
    final QuerySnapshot docRef =
        await FirebaseFirestore.instance.collection('projects').doc(GlobalConfiguration().getValue('projectId')).collection('challenges').where('segment_id', isEqualTo: segmentId).where('user.id', isEqualTo: userId).get();
    if (docRef.docs.isNotEmpty) {
      return docRef.docs.map((challengeData) {
        final data = challengeData.data() as Map<String, dynamic>;
        return Challenge.fromJson(data);
