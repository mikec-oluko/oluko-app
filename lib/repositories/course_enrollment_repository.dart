...
class CourseEnrollmentRepository {
  ...
  static Future<CourseEnrollment> get(Course course, User user) async {
    ...
    if (qs.docs.isNotEmpty) {
      return CourseEnrollment.fromJson(
  ...
  static Future<List<CourseEnrollment>> getByCourse(
  ...
  static Future<void> markSegmentAsCompleted(CourseEnrollment courseEnrollment,
  ...
  static Future<CourseEnrollment> create(User user, Course course) async {
    final DocumentReference projectReference = FirebaseFirestore.instance
        .collection('projects')
        .doc(GlobalConfiguration().getValue('projectId'));
  ...
  static List<EnrollmentSection> getEnrollmentSections(
      SegmentSubmodel segment) {
    final List<EnrollmentSection> sections = [];
    if (segment.sections != null) {
      for (final section in segment.sections) {
        sections
            .add(EnrollmentSection(movements: getEnrollmentMovements(section)));
      }
    }
    return sections;
  }

  static List<EnrollmentMovement> getEnrollmentMovements(
      SectionSubmodel section) {
    final List<EnrollmentMovement> movements = [];
    for (final movement in section.movements) {
      movements.add(EnrollmentMovement(
          id: movement.id, reference: movement.reference, name: movement.name));
    }
    return movements;
  }
  ...
  static Future<void> saveMovementCounter(
    ...
    final List<EnrollmentMovement> movements = classes[classIndex]
