...
class SegmentUtils {
  static List<Widget> getSegmentSummary(Segment segment, BuildContext context) {
    ...
    return [getRoundTitle(segment, context, OlukoColors.grayColor), SizedBox(height: 12.0)] + workoutWidgets;
  }
  ...
  static List<Widget> getWorkouts(Segment segment, BuildContext context) {
    ...
    segment.movements.forEach((MovementSubmodel movement) {
      if (movement.timerReps != null) {
        ...
        workoutString = movement.timerWorkTime.toString() + 's ' + movement.name;
      }
  ...
  static List<TimerEntry> getExercisesList(Segment segment, BuildContext context) {
    if (hasRest(segment)) {
  ...
  static List<TimerEntry> getIndividualExercisesList(Segment segment) {
    ...
    for (var roundIndex = 0; roundIndex < segment.rounds; roundIndex++) {
      for (var movementIndex = 0; movementIndex < segment.movements.length; movementIndex++) {
        ...
        for (var setIndex = 0; setIndex < cantSets; setIndex++) {
          bool isTimedEntry = segment.movements[movementIndex].timerWorkTime != null;
          ...
          if (hasRest) {
            ...
            entries.add(TimerEntry(
                ...
                setNumber: hasSets ? setIndex + 1 : null,
                roundNumber: roundIndex + 1,
                label: '${segment.movements[movementIndex].timerRestTime}s rest',
          ...
          if (isLastMovement && segment.roundBreakDuration != null) {
            ...
            entries.add(TimerEntry(
                ...
                setNumber: hasSets ? setIndex + 1 : null,
                roundNumber: roundIndex + 1,
                label: '${isLastMovement ? segment.roundBreakDuration : segment.movements[movementIndex].timerRestTime}s rest',
                workState: WorkState.resting));
          }
        }
      }
  ...
  static List<TimerEntry> getJoinedExercisesList(Segment segment, BuildContext context) {
    List<TimerEntry> entries = [];
    ...
    for (var roundIndex = 0; roundIndex < cantRounds; roundIndex++) {
      ...
      entries.add(TimerEntry(
          ...
          reps: segment.timerType == TimerTypeEnum.combined ? segment.movements[0].timerReps : null,
  ...
  static List<String> getMovements(Segment segment, BuildContext context) {
    ...
    segment.movements.forEach((MovementSubmodel movement) {
      if (movement.timerReps != null) {
        movementStrings.add(movement.timerReps.toString() + 'x ' + movement.name);
      } else {
        movementStrings.add(movement.timerWorkTime.toString() + 's ' + movement.name);
      }
  ...
  static List<Widget> getJoinedMovements(Segment segment, BuildContext context) {
