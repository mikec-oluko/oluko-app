...
class _ProfileMyAccountPageState extends State<ProfileMyAccountPage> {
  ...
  Padding getTextFormField(String value, String title, String oldUserName,
      String oldEmail, String key) {
    return Padding(
      padding: const EdgeInsets.only(
          left: OlukoNeumorphism.isNeumorphismDesign ? 20 : 10),
      child: TextFormField(
    ...
    return BlocListener<CountryBloc, CountryState>(
      listener: (context, state) {
        if (state is CountrySuccess) {
          if (countries == null ||
              countries.isEmpty ||
              _profileInfo.country != newFields.country) {
            setState(() {
      ...
      child: countries != null && countries.isNotEmpty
          ? Padding(
              ...
              child: DropdownButton(
                dropdownColor: OlukoNeumorphism.isNeumorphismDesign
                    ? OlukoNeumorphismColors.olukoNeumorphicGreyBackgroundFlat
                    : Colors.transparent,
                isExpanded: true,
                value: newFields.country ??
                    ...
                    countries[0].name,
                items:
                    countries.map<DropdownMenuItem<String>>((Country country) {
                ...
                onChanged: (String item) async {
                  final selectedCountry =
                      countries.firstWhere((country) => country.name == item);
                  final List<String> statesOfSelectedCountry =
                      selectedCountry?.states;
                  var newFieldsState = newFields.state;
                  var newCountries = countries;
                  if (statesOfSelectedCountry != null &&
                      statesOfSelectedCountry.isNotEmpty) {
                    newFieldsState = statesOfSelectedCountry[0];
                  } else {
                    newCountries = await BlocProvider.of<CountryBloc>(context)
                        .getStatesForCountry(selectedCountry.id);
                    final Country newCountryWithStates =
                        newCountries.firstWhere(
                            (element) => element.id == selectedCountry.id);
                    newFieldsState = newCountryWithStates != null &&
                            newCountryWithStates.states != null
  ...
  List<DropdownMenuItem<String>> getItemsForStatesDropdown() {
    ...
    if (newFields.country != null) {
      statesList = countries
          .firstWhere((country) => country.name == newFields.country)
          ?.states;
    } else if (_profileInfo.country != null) {
      statesList = countries
          .firstWhere((country) => country.name == _profileInfo.country)
          ?.states;
    } else {
